...
from conversion import dB2Linear, linear2dB
...
class TdlChannel:
    ...
    def corrupt_data(self, signal: np.ndarray) -> np.ndarray:
        ...
        signal = torch.from_numpy(self.__prepare_transmit_signal_shape(signal)).to(device)
        ...
        tap_values_sparse = torch.from_numpy(self._last_impulse_response.tap_values_sparse).to(device)
        ...
        if len(self._fading_generator.shape) == 1:
            ...
            if self.switched_direction:
                ...
                output = torch.zeros((num_rx_ant, num_symbols + channel_memory), dtype=torch.cfloat, device=device)
