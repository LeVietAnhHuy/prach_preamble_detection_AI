...
for snr_dB in tqdm(snr_dB_range):
    ...
    while num_sample <= num_sample_per_snr:
        for preamble_index in range(1, 64):
            ...
            x_u_fft = np.matlib.repmat(x_u_fft_mat[preamble_index], random_access_config.prachDuration, num_rx_antennas)
            x_u_fft = np.reshape(x_u_fft, (num_rx_antennas, random_access_config.prachDuration, -1))
            ...
            for i in range(x_corr_ifft.shape[0]):
                for j in range(x_corr_ifft.shape[1]):
                    ...
                    full_image_path_corr = os.path.join(image_dir, 'main.png')
                    plt.savefig(full_image_path_corr, dpi=300)
                    print(f"Plot saved as '{full_image_path_corr}'")
                    plt.close()
